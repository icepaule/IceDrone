...
For the temporary perfboard build use **[`docs/10_PERFBOARD_V34_SOLDERING_DE.md`](docs/10_PERFBOARD_V34_SOLDERING_DE.md)** and the machine-readable files in `hardware/`.
...
1. Read [`docs/01_BOM.md`](docs/01_BOM.md) and [`docs/02_ELECTRICAL.md`](docs/02_ELECTRICAL.md); V3.4 is authoritative.
2. If building the temporary 70×30 mm perfboard, follow [`docs/10_PERFBOARD_V34_SOLDERING_DE.md`](docs/10_PERFBOARD_V34_SOLDERING_DE.md) hole by hole and multimeter-test each stage.
3. Print [`cad/airframe_v2/stl/motor_cup_test_v2.stl`](cad/airframe_v2/stl/motor_cup_test_v2.stl) before committing to the full frame.
4. Print the Airframe V2 parts using the settings in [`docs/03_MECHANICAL.md`](docs/03_MECHANICAL.md).
5. Assemble and power electronics **without propellers**, following [`docs/05_BUILD_AND_TEST.md`](docs/05_BUILD_AND_TEST.md).
6. Run the V3.4 bench firmware in [`firmware/bench_test/`](firmware/bench_test/) and verify IMU orientation and every motor channel.
7. Only after all bench checks pass, install balanced propellers and continue with [`docs/06_FIRST_FLIGHT.md`](docs/06_FIRST_FLIGHT.md).
